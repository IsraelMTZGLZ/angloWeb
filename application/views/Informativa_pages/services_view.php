
      <div class="page_title_section float_left">

          <div class="page_header">
              <div class="container">
                  <div class="row">
                      <!-- section_heading start -->
                      <div class="col-lg-12 col-md-12 col-12 col-sm-12">

                          <h1><?=$this->lang->line('nuestros_servicios');?></h1>
                      </div>
                      <div class="col-lg-12 col-md-12 col-12 col-sm-12">
                          <div class="sub_title_section">
                              <ul class="sub_title">

                              </ul>
                          </div>
                      </div>
                  </div>
              </div>
          </div>
      </div>
  </div>

  <!-- navi wrapper End -->
<!-- services_section start -->
<div class="services_section float_left">
      <div class="container">
          <div class="row">
              <div class="col-lg-12 col-md-12">
                  <div class="row">
                      <div class="col-md-4">
                          <div class="main_wrapper">
                              <i class="flaticon-eye-open"></i>
                              <div class="inner_wrapper">
                                  <h4><a href="javascript:;"><?=$this->lang->line('ayuda_servicio');?></a></h4>
                                  <p><?=$this->lang->line('ayudar_servicio_contenido');?></p>
                              </div>
                          </div>
                      </div>
                      <div class="col-md-4">
                          <div class="main_wrapper">
                               <i class="flaticon-triangular-arrows-sign-for-recycle"></i>
                              <div class="inner_wrapper">
                                  <h4><a href="javascript:;"><?=$this->lang->line('orientecion_servicio');?></a></h4>
                                  <p><?=$this->lang->line('orientecion_servicio_contenido');?></p>
                              </div>
                          </div>
                      </div>
                      <div class="col-md-4">
                          <div class="main_wrapper">
                              <i class="flaticon-trophy"></i>
                              <div class="inner_wrapper">
                                  <h4><a href="javascript:;"><?=$this->lang->line('garantizar_servicio');?></a></h4>
                                  <p><?=$this->lang->line('garantizar_servicio_contenido');?>.</p>
                              </div>
                          </div>
                      </div>
                      <div class="col-md-4">
                          <div class="main_wrapper">
                               <i class="flaticon-shopping-cart"></i>
                              <div class="inner_wrapper">
                                  <h4><a href="javascript:;"><?=$this->lang->line('asesoria_servicio');?></a></h4>
                                  <p><?=$this->lang->line('asesoria_servicio_contenido');?></p>
                              </div>
                          </div>
                      </div>
                      <div class="col-md-4">
                          <div class="main_wrapper">
                              <i class="flaticon-portfolio"></i>
                              <div class="inner_wrapper">
                                  <h4><a href="javascript:;"><?=$this->lang->line('analisis_servicio');?></a></h4>
                                  <p><?=$this->lang->line('analisis_servicio_contenido');?></p>
                              </div>
                          </div>
                      </div>
                      <div class="col-md-4">
                          <div class="main_wrapper">
                             <i class="flaticon-graduate-cap"></i>
                              <div class="inner_wrapper">
                                  <h4><a href="javascript:;"><?=$this->lang->line('gratis_servicio');?></a></h4>
                                  <p><?=$this->lang->line('gratis_servicio_contenido');?></p>
                              </div>
                          </div>
                      </div>
                  </div>
              </div>
          </div>
      </div>
  </div>
  <!-- services_section end -->
  <!--process wrapper start-->
  <div class="process_wrapper float_left">
      <div class="container">
          <div class="row">
              <div class="col-lg-12 col-md-12 col-sm-12 col-12">

                  <div class="cp_heading_wraper">

                      <h3><?=$this->lang->line('step_title_service');?></h3>

                      <p> <?=$this->lang->line('step_title_service_detail');?></p>
                  </div>

              </div>
              <div class="work_process_main_wrapper float_left">
                  <div class="work_process_wrapper">
                      <div class="work_box float_left">
                          <div class="work_img"><img src="<?=base_url('resources/assets/Informativa/images/London_TRABELTHREE.png');?>" alt="img" /></div>
                          <h1 class="numbering">1</h1>
                          <h2><a href="Oferta"><?=$this->lang->line('step_one_service');?></a></h2>
                          <p><?=$this->lang->line('step_one_service_detail');?></p>
                          <!-- <p><?=$this->lang->line('step_one_service_detail_two');?></p> -->
                      </div>
                  </div>
                  <div class="work_process_wrapper p2">
                      <div class="work_box float_left">
                          <div class="work_img"><img src="<?=base_url('resources/assets/Informativa/images/London_travelTwo.jpg');?>" alt="img" /></div>
                          <h1 class="numbering numbering2">2</h1>
                          <h2><a href="Contacto"><?=$this->lang->line('step_two_service');?></a></h2>
                          <p><?=$this->lang->line('step_two_service_detail');?></p>
                      </div>
                  </div>
                  <div class="work_process_wrapper p3">
                      <div class="work_box float_left">
                          <div class="work_img"><img src="<?=base_url('resources/assets/Informativa/images/london_travelOne.jpg');?>" alt="img" /></div>
                          <h1 class="numbering numbering3">3</h1>
                          <h2><a href="Login"><?=$this->lang->line('step_three_service');?></a></h2>
                          <p><?=$this->lang->line('step_three_service_detail');?></p>
                      </div>

                  </div>
                  <div class="work_process_wrapper">
                      <div class="work_box float_left">
                          <div class="work_img"><img src="<?=base_url('resources/assets/Informativa/images/london_travelk.png');?>" alt="img" /></div>
                          <h1 class="numbering numbering4">4</h1>
                          <h2><a href="javascript:;"><?=$this->lang->line('step_four_service');?></a></h2>
                          <p><?=$this->lang->line('step_four_service_detail');?> </p>
                      </div>

                  </div>
              </div>
          </div>
      </div>
  </div>
  <!--process wrapper end-->
   
  <!--offer wrapper end-->
  <!--pricing wrapper start-->

  <!--pricing wrapper end-->
  <!--newsletter wrapper start-->

  <div class="news_letter_wrapper float_left">

        <div class="container">

            <div class="lr_nl_heading_wrapper">
                <h2><?=$this->lang->line('nuevo_about')?></h2>
                <p><?=$this->lang->line('nuevo_leyenda_about')?></p>
            </div>

            <div class="lr_nl_form_wrapper">
                <button>
                    <a href="Login"><?=$this->lang->line('suscribete_about')?></a>
                </button>
            </div>

        </div>
    </div>
