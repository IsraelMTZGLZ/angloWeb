...
class InfoAspiranteDosR extends MY_RootController {
    ...
    $fkInstThree =$aux_selec_three['fkInstituto'];
		...
		if($responseNumAplicanteThree['data']!=NULL){
		 ...
		 if($lenInstitutos == $lenDocsAcepted  and $lenInstitutos == $lenDocsAcepted and $lenDocsAceptedNum==$lenInstitutos and 3==4){
			 // echo('Si');
			$data= array(
				"status"=>"3"
			);
    ...
    $responseFile = $this->_callApiRest('Verano/Ingles/api/fileinfoVisa/id/'.$idAspirante,null,"GET",null);
		...
		$responseDocumentosAcademico = $this->_callApiRest('Verano/Academico/api/documentosVeranoAcademico/id/'.$idAspirante,null,"GET",null);
